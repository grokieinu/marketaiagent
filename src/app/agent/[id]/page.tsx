...
export default function AgentDetailPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Convert attachments to base64 for sending to agent
      ...
      if (!res.ok) {
        if (data.needCredits) {
          setError(`Insufficient credits. Need ${data.required}, have ${data.available}. Buy more on Credits page.`)
        } else {
      ...
      <div className="min-h-screen py-12">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
              <div className="p-3 rounded-lg bg-white/5">
                ...
                <p className="text-lg font-bold text-green-400">{isFree ? 'Free' : `${agent.price} credits`}</p>
              </div>
              <div className="p-3 rounded-lg bg-white/5">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {!isFree && (
                <span className="px-3 py-1 rounded-full text-xs bg-yellow-500/10 border border-yellow-500/20 text-yellow-400">
                  {agent.price} credits per request
                </span>
              )}
          ...
          {!isFree && !connected && (
            ...
            >
              <p className="text-sm text-yellow-300 mb-3">This agent costs {agent.price} credits per request. Connect wallet to use.</p>
            ...
            <div className="mb-5">
              <div className="p-5 rounded-xl bg-white/5 border border-white/10">
                <div className="flex items-center justify-between mb-3">
                  ...
                  {/* Edit/Refine button */}
                  <button
                    onClick={() => { setPrompt(`Refine the previous result: ${response.slice(0, 100)}...\n\nMy changes: `); }}
              ...
              <Sandbox content={response} isFree={isFree} hasPaid={isFree} onRequestDownload={() => setError('Buy credits to download this content. Go to Credits page.')} />
            </div>
          )}
