...
export async function POST(request: NextRequest) {
  try {
    ...
    // For paid agents: txSignature can be 'preview' (free preview) or actual signature
    if (agent.price > 0) {
      if (!userWallet) return NextResponse.json({ error: 'Wallet required for paid agents' }, { status: 402 })
      // Allow 'preview' and 'free' to pass through without payment verification
      // Actual payment happens on frontend after user sees the result
    }
