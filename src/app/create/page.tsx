          ...
          {step === 1 && (
            <div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">Price per Request (Credits)</label>
                  <input
                    type="number"
                    value={formData.price}
                    onChange={(e) => updateForm('price', e.target.value)}
                    placeholder="0 for free, or e.g. 5"
                    step="1"
                    min="0"
                    className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-primary-500/50"
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    Set to 0 for free (no wallet required). Paid agents charge credits per request. Users buy credits with SOL.
                  </p>
                </div>
              </div>
            </div>
                ...
                <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-white font-medium">{formData.price ? `${formData.price} credits` : 'Free'}</p>
                    </div>
                ...
                <div className="p-4 rounded-xl bg-green-500/5 border border-green-500/20">
                  ...
                  <div className="space-y-2 text-sm text-gray-400">
                    <div className="flex justify-between">
                      ...
                      <span className="text-white font-medium">{formData.price || '0'} credits</span>
                    </div>
                    <div className="flex justify-between">
                      <span>You receive (95%)</span>
                      <span className="text-green-400 font-medium">{formData.price ? (parseFloat(formData.price) * 0.95).toFixed(0) : '0'} credits</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Platform fee (5%)</span>
                      <span className="text-gray-500">{formData.price ? (parseFloat(formData.price) * 0.05).toFixed(0) : '0'} credits</span>
                    </div>
                  </div>
                </div>
